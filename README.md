...
| --- | --- |
| `/` | 已发布文章列表，包含摘要、封面和标签 |
| `/posts` | 全部文章归档，含发布时间和阅读时长 |
| `/timeline` | 按日期浏览公开文章 |
| `/posts/:slug` | 文章详情，可通过 slug 直接访问，支持一键复制 Markdown 原文 |
| `/notes` | 按日锚定的轻量笔记：公开页 + 登录管理（编辑器/归档/标签/日历筛选/搜索） |
| `/admin` | 登录后管理文章：创建、编辑、预览、发布、撤回、删除 |
| `/login` | 邮箱密码登录 |
| `/tools/image-editor` | 浏览器本地图片编辑器；图片不会上传服务器 |
| `/tools/text-cards` | 登录专属卡片笔记（MVP 为列表式编辑）；数据存 PostgreSQL，支持多工作区与 v1 JSON 备份 |
