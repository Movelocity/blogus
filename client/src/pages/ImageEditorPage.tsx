import { ArrowClockwise, ArrowCounterClockwise, ArrowLeft, ArrowsOut, CaretRight, DownloadSimple, ImageSquare, LinkSimple, LinkSimpleBreak, Minus, Plus, UploadSimple } from "@phosphor-icons/react";
...
export function ImageEditorPage() {
  ...
  useEffect(() => setPanelOpen(Boolean(selected)), [selected?.id]);
  ...
  const panel = <aside className="relative flex h-full w-full flex-col gap-5 bg-sidebar/75 p-4 backdrop-blur-md md:p-5">
    <button type="button" onClick={() => { setPanelOpen(false); dispatch({ type: "select", id: null }, false); }} className="absolute right-3 top-3 rounded-md border bg-background/70 p-1.5 hover:bg-muted" aria-label="折叠侧边栏" title="折叠侧边栏"><CaretRight size={18} /></button>
  ...
  return <div className="flex h-[100dvh] flex-col overflow-hidden bg-background text-foreground">
    ...
    <div className="relative flex min-h-0 flex-1 md:flex-row"><main className="relative min-h-0 min-w-0 flex-1 overflow-hidden bg-muted/35"><div className="absolute left-1/2 top-3 z-40 flex -translate-x-1/2 items-center gap-1 rounded-lg border bg-background/95 p-1 shadow-sm backdrop-blur"><button aria-label="缩小" onClick={() => setZoomAt(zoomRef.current / 1.2)} className="rounded p-1.5 hover:bg-muted"><Minus /></button><button title="重置缩放" onClick={() => setZoomAt(1)} className="min-w-14 rounded px-2 py-1 text-xs hover:bg-muted">{Math.round(zoom * 100)}%</button><button aria-label="放大" onClick={() => setZoomAt(zoomRef.current * 1.2)} className="rounded p-1.5 hover:bg-muted"><Plus /></button><button title="适应全部" onClick={fit} className="rounded p-1.5 hover:bg-muted"><ArrowsOut /></button></div>
      ...
      <div className={`absolute inset-y-0 right-0 z-50 w-[min(88vw,18rem)] overflow-auto border-l shadow-[-12px_0_32px_rgba(0,0,0,.12)] transition-transform duration-200 ease-out ${selected && panelOpen ? "translate-x-0" : "pointer-events-none translate-x-full"}`}>{panel}</div></div>
  </div>;
}
